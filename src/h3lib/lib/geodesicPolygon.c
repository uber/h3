...
static bool _geodesicEdgesCross(const Vec3d *a1, const Vec3d *a2,
    ...
    intersectionLine = vec3Cross(normalA, *normalB);

    if (vec3NormSq(intersectionLine) < EPSILON * EPSILON) {
        const Vec3d refDir = {a2->x - a1->x, a2->y - a1->y, a2->z - a1->z};
        if (vec3NormSq(refDir) < EPSILON * EPSILON) {
            return false;
        }
    ...
    if (vec3NormSq(midASum) < EPSILON * EPSILON ||
        vec3NormSq(midBSum) < EPSILON * EPSILON) {
        return true;
    }
...
static bool _candidateContainsVerts(const Vec3d *candidate,
    ...
    if (vec3NormSq(*candidate) < EPSILON * EPSILON) {
        return false;
    }
...
static bool _geoLoopVerticesFitHemisphere(const GeodesicEdge *edges,
    ...
    for (int i = 0; i < numEdges; i++) {
        for (int j = i + 1; j < numEdges; j++) {
            Vec3d candidate;
            candidate = vec3Cross(edges[i].vert, edges[j].vert);
...
static bool _geoLoopVerticesOnGreatCircle(const GeodesicEdge *edges,
    ...
    for (int i = 0; i < numEdges && !foundNormal; i++) {
        for (int j = i + 1; j < numEdges; j++) {
            normal = vec3Cross(edges[i].vert, edges[j].vert);
            if (vec3NormSq(normal) > 1e-12) {
                foundNormal = true;
                break;
            }
        }
...
static H3Error _geodesicLoopFromGeo(const GeoLoop *loop, GeodesicLoop *out,
    ...
    *out = (GeodesicLoop){0};
    ...
    for (int i = 0; i < n; i++) {
        ...
        edges[i].vert = latLngToVec3(loop->verts[i]);
    }
