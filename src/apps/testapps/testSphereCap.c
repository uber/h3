...
static bool _cellWithinCap(const H3Index cell, const SphereCap *cap) {
    ...
    for (int i = 0; i < boundary.numVerts; i++) {
        Vec3d vertex;
        vertex = latLngToVec3(boundary.verts[i]);
        ...
        Vec3d next;
        next = latLngToVec3(boundary.verts[(i + 1) % boundary.numVerts]);
        Vec3d midpoint = {vertex.x + next.x, vertex.y + next.y,
                          vertex.z + next.z};
        double mag = vec3Norm(midpoint);
        if (mag > EPSILON) {
            midpoint.x /= mag;
            midpoint.y /= mag;
            midpoint.z /= mag;
...
SUITE(SphereCap) {
    TEST(cellToSphereCapOutputs) {
        ...
        t_assert(fabs(vec3Norm(cap.center) - 1.0) < 1e-12,
