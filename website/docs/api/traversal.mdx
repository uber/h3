...
```c
H3Error cellToLocalIj(H3Index origin, H3Index h3, uint32_t flags, CoordIJ *out);
```
...
`flags` is reserved for future expansion and must be set to `0`.
...
```c
H3Error localIjToCell(H3Index origin, const CoordIJ *ij, uint32_t flags, H3Index *out);
```
