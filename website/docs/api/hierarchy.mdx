...
Populates `children` with the indexes contained by `cell` at resolution `childRes`. `children` must be an array of at least size `cellToChildrenSize(h, childRes)`.
...
```c
int64_t uncompactCellsSize(const H3Index* compactedSet, const int64_t numCompacted, const int res)
```
