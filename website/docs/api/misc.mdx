...
```c
double pointDistKm(const LatLng *a, const LatLng *b);
```
...
```java
double pointDist(LatLng point1, LatLng point2, LengthUnit unit);
```
...
function example() {
  const point1 = [-10, 0];
  const point2 = [10, 0];
  return h3.pointDist(point1, point2, h3.UNITS.km)
}
```
...
```c
double pointDistM(const LatLng *a, const LatLng *b);
```
...
function example() {
  const point1 = [-10, 0];
  const point2 = [10, 0];
  return h3.pointDist(point1, point2, h3.UNITS.m)
}
```
...
```c
double pointDistRads(const LatLng *a, const LatLng *b);
```
