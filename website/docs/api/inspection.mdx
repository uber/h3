...
```java
long constructCell(int res, int baseCellNumber, List<Integer> digits);
String constructCellAddress(int res, int baseCellNumber, List<Integer> digits);
```
...
```sql
h3_construct_cell(res, base_cell_number, digits)
h3_construct_cell_string(res, base_cell_number, digits)
```
