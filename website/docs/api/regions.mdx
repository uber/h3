...
When geodesic traversal is enabled, only `FULL` and `OVERLAPPING`
containment modes are valid. `CENTER` and `OVERLAPPING_BBOX` are invalid in
geodesic mode.
...
When geodesic mode is enabled in Python, only `full` and `overlap` are valid.
...
When geodesic mode is enabled in Go, only `h3.ContainmentFull` and
`h3.ContainmentOverlapping` are valid.
...
When geodesic mode is enabled in DuckDB, only `full` and `overlap` are valid.
